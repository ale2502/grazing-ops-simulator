...
## 2026-07-14 - Command Persistence and Dependency Injection

- Focus: Complete the next `scheduleHerdMove` TDD slice and understand the TypeScript syntax used to describe injected function and repository dependencies.
- Decisions: `scheduleHerdMove` now creates a complete `HerdMoveCommand`, saves it through `commandRepository.create`, then creates and saves one pending acknowledgement per collar. The command is saved first because acknowledgement records refer to its ID. The three unit tests remain separated by behaviour: dispatched status, per-collar acknowledgements, and command persistence.
- What I learned: The persistence test followed red-green TDD: it first failed with an empty `createdCommands` array, proving it detected the missing repository call, and then passed after the smallest production change. A dependency is a reusable tool a function needs but does not construct itself. `input` describes the particular herd-move job, while `dependencies` supplies tools such as ID generation and repository operations. A TypeScript type containing method signatures describes the required shape of those tools; it does not create or execute the functions. Fake repositories let unit tests control inputs and record outputs without PostgreSQL, while later production repositories can satisfy the same contract with real database operations.
- Corrections: `createId(): string` means `createId` is a no-argument function that returns a string, not a string property. `Promise<Collar[]>` means an asynchronous function eventually returns an array of collars, while `Promise<void>` means asynchronous work completes without a useful return value. `scheduleHerdMove` is already the real business function; only its test repository implementations are fake.
- Open questions: The current unit tests do not yet prove that a real database repository can persist these records, and database integration should wait until the business contract and next small backend slices are understood.
- Next practice task: Explain one `ScheduleHerdMoveDependencies` property in your own words, then point to the matching fake implementation in `scheduleHerdMove.test.ts` and the place where `scheduleHerdMove.ts` calls it.
